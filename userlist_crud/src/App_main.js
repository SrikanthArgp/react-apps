...
export default function Home() {
  ...
  const [fireData, setFireData] = useState(JSON.parse(localStorage.getItem('userlist')) || []);
  const [isUpdate, setIsUpdate] = useState(false);

  useEffect(() => {
    localStorage.setItem('userlist', JSON.stringify(fireData));
  }, [fireData]);
  ...
  const updateFields = () => {
    const selectedItem = fireData.find((item) => item.id === ID);
    const updateItem = fireData.map((e) =>
  ...
  const deleteDocument = (id) => {
    setFireData((prevData) => {
      return fireData.filter((item) => item.id !== id);
    });
  };
